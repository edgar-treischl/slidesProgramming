...
- **Functions as first-class objects** – you can assign functions to variables, pass them as arguments, and return them from other functions.  
- **Anonymous functions** – functions without names, useful for inline transformations.  
- **Apply family** – `lapply()`, `sapply()`, `vapply()`, enable iteration without loops.  
- **Closures / lexical scoping** – functions that capture and retain their environment, allowing reusable workflows.  
- **Pure functions** – functions without side effects, producing predictable results.  
...
# You can assign a function to a variable and call it like any other function
f <- sum       
f(1:5)
...
```{r}
# Closures: Functions that return other functions, capturing their environment
...
background-position: 90% 5%
background-size: 8%
## purrr::map2 – iterating over two inputs in parallel

.pull-left[

`map2()` is used when you want to **apply a function to two vectors or lists at the same time**, element by element. 

It’s perfect for combining, comparing, or transforming two inputs together without writing explicit loops. So, `map2()` takes three main arguments:  
...
map2_chr(column_names, column_means, ~ paste(.x, "mean =", round(.y, 1)))
...
background-position: 90% 5%
background-size: 8%
## purrr::pmap – iterating over multiple inputs
