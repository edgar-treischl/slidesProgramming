...
/images/preview.png",
    image_alt = "R Programming Slides",
  )
```
...
#### `r emoji::emoji("toolbox")` Create your own functions
...
#### `r emoji::emoji("sponge")` Pure functions:
...
#### `r emoji::emoji("cyclone")` Closures/lexical scoping:
...
#### `r emoji::emoji("gear")` Apply family
...
#### `r emoji::emoji("point_right")` Sapply makes it hard to predict the output
...
#### `r emoji::emoji("repeat")` While-loop
...
#### `r emoji::emoji("repeat_one")` For-loop
...
#### `r emoji::emoji("input_numbers")` Vectorization
...
#### `r emoji::emoji("chains")` Pipe operator (`%>%`)
...
# .white[Purrr your way ... <br> `r emoji::emoji("cat")`] 
...
#### `r emoji::emoji("point_right")` `pmap()` allows you to work cleanly with **any number of parallel inputs**, keeping your code short and readable while avoiding nested loops.
...
#### `r emoji::emoji("point_right")` Think of it as **map over columns** — the *column-wise equivalent* of `map()`.
...
`r emoji::emoji("point_right")` You can select columns using other **tidyselect** helpers, such as: `starts_with`, `ends_with`, `contains`, `matches` `everything`.
...
[`r icons::fontawesome("file-pdf")` Slides](https://github.com/edgar-treischl/slidesProgramming/blob/main/r_programming.pdf)
